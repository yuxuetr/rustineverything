...
pub fn read_episode_from_dir(dir: &std::path::Path) -> Option<Episode> {
    ...
    let yaml_file = if yaml_path.exists() { Some(yaml_path) }
        else if yml_path.exists() { Some(yml_path) }
        else { None };
...
pub fn scan_episodes(root: &std::path::Path) -> Vec<Episode> {
    ...
    episodes.sort_by(|a, b| {
        ...
        if by_date == std::cmp::Ordering::Equal { b.id.cmp(&a.id) } else { by_date }
    });
    episodes
}
...
mod tests {
    ...
    fn test_scan_episodes_sorted_by_date_desc() {
        ...
        write_yaml(&root.join("a"), "id: 1\ntitle: A\nduration: \"10:00\"\ndate: 2024-01-01\naudio_url: a.mp3\n");
        write_yaml(&root.join("b"), "id: 2\ntitle: B\nduration: \"10:00\"\ndate: 2024-03-15\naudio_url: b.mp3\n");
        write_yaml(&root.join("c"), "id: 3\ntitle: C\nduration: \"10:00\"\ndate: 2024-02-10\naudio_url: c.mp3\n");
    ...
    fn test_scan_skips_underscore_and_hidden() {
        ...
        write_yaml(&root.join("visible"), "id: 1\ntitle: V\nduration: \"5:00\"\ndate: 2024-01-01\naudio_url: a.mp3\n");
        write_yaml(&root.join("_draft"),  "id: 2\ntitle: D\nduration: \"5:00\"\ndate: 2024-01-02\naudio_url: d.mp3\n");
        write_yaml(&root.join(".hidden"), "id: 3\ntitle: H\nduration: \"5:00\"\ndate: 2024-01-03\naudio_url: h.mp3\n");
    ...
    fn test_scan_skips_dirs_without_yaml_or_audio() {
        ...
        write_yaml(&root.join("withyaml"), "id: 1\ntitle: T\nduration: \"5:00\"\ndate: 2024-01-01\naudio_url: a.mp3\n");

...
#[cfg(feature = "server")]
pub(crate) fn scan_doc_dir(dir: &std::path::Path, rel_prefix: &str, depth: u32) -> Vec<DocTreeNode> {
...
pub async fn get_doc_content(path: String) -> Result<DocContentResponse, ServerFnError> {
    ...
    #[cfg(not(feature = "server"))]
    { let _ = path; Ok(DocContentResponse { content: String::new(), meta: DocMeta::default() }) }
}

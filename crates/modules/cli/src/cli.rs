...
pub fn CliIndexPage() -> Element {
  let articles_res = use_resource(|| async move { list_cli_articles().await.unwrap_or_default() });
                      ...
                      match articles_res.read().as_ref() {
                          ...
                          Some(_) if filtered.is_empty() => rsx! {
                              div { class: "py-16 text-center text-slate-400",
                                  "暂无文章。把 markdown 放到 assets/topics/cli/<slug>/index.md 即可。"
                              }
                          },
                          Some(_) => rsx! {
                              div { class: "space-y-4",
...
pub fn CliArticlePage(slug: String) -> Element {
  ...
  let content_res = use_resource(move || {
    ...
    async move { get_cli_article(s).await }
  });

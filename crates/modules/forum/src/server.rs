...
pub async fn list_tags() -> Result<Vec<TagSummary>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        use sea_orm::{
            ColumnTrait, EntityTrait, FromQueryResult, Order, QueryOrder, QuerySelect,
        };
...
pub async fn post_reply(topic_id: i32, content: String) -> Result<TopicDetail, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        topic::Entity::update_many()
            .col_expr(
                topic::Column::ReplyCount,
                Expr::col(topic::Column::ReplyCount).add(1),
            )
...
pub async fn list_my_topics() -> Result<Vec<TopicSummary>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        Ok(rows
            .into_iter()
            .map(|t| topic_to_summary(t, me.as_ref()))
            .collect())
    }
    #[cfg(not(feature = "server"))]
    {

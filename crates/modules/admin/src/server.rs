...
pub async fn admin_overview() -> Result<AdminOverview, ServerFnError> {
    #[cfg(feature = "server")]
    {
        ...
        let user_count = user_entity::Entity::find()
            .count(&db)
            .await
        ...
        let comment_count = comment::Entity::find()
            ...
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))? as i64;
        let topic_count = topic::Entity::find()
            .count(&db)
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))? as i64;
        let reply_count = topic_reply::Entity::find()
            .count(&db)
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))? as i64;
        let annotation_count = annotation::Entity::find()
            .count(&db)
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))? as i64;
...
pub async fn admin_list_users(
    page: Option<u32>,
) -> Result<AdminPage<AdminUserRow>, ServerFnError> {
    #[cfg(feature = "server")]
    {
...
pub async fn admin_set_user_role(
    ...
    #[cfg(feature = "server")]
    {
        ...
        use rustineverything_core::session::require_admin;
        use sea_orm::{
            ActiveValue::Set, ColumnTrait, EntityTrait, PaginatorTrait, QueryFilter,
        };

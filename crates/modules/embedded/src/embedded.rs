//! 嵌入式板块的落地页与文章详情页。
//!
//! 导航一律用 `<a href>`（而非 app 的 `Route`），避免对 app crate 的循环依赖
...
/// 落地页：子主题筛选 + 搜索 + 文章卡片 + 精选 crate 侧栏。
#[component]
pub fn EmbeddedIndexPage() -> Element {
    let articles_res = use_resource(|| async move { list_embedded_articles().await.unwrap_or_default() });
...
pub fn EmbeddedArticlePage(slug: String) -> Element {
    ...
    let content_res = use_resource(move || {
        ...
        async move { get_embedded_article(s).await }
    });

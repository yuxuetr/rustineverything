//! AI 板块的落地页与文章详情页。导航用 `<a href>`，避免对 app `Route` 的循环依赖。
//!
...
use crate::server::{get_ai_article, list_ai_articles, ArticleSummary};
use crate::text::{
...
/// 文章列表（重构 B3）：`list_ai_articles` 经 `use_server_future` 服务端预取，
/// 子主题筛选 / 搜索仍是客户端 signal 交互。置于 SuspenseBoundary 内。
#[component]
fn AiIndexList() -> Element {
  let articles_res =
    use_server_future(|| async move { list_ai_articles().await.unwrap_or_default() })?;
          ...
          div { class: "lg:col-span-2",
              if filtered.is_empty() {
                  div { class: "py-16 text-center text-slate-400",
                      "暂无文章。把 markdown 放到 assets/topics/ai/<slug>/index.md 即可。"
                  }
              } else {
                  div { class: "space-y-4",
...
#[component]
pub fn AiArticlePage(slug: String) -> Element {
  rsx! {
      section { class: "py-12 bg-white dark:bg-slate-950",
          div { class: "max-w-4xl mx-auto px-4 sm:px-6",
              ...
              div { class: "text-slate-700 dark:text-slate-200",
                  ...
                  SuspenseBoundary {
                      ...
                      AiArticleContent { slug: slug.clone() }
                  }
              }
          }
      }
...
/// 文章正文（重构 B3）：`get_ai_article` 经 `use_server_future` + `use_reactive!` 服务端预取，
/// 随路由参数 slug 变化重取。置于 SuspenseBoundary 内。
#[component]
fn AiArticleContent(slug: String) -> Element {
  let content_res =
    use_server_future(use_reactive!(|slug| async move { get_ai_article(slug).await }))?;
  match content_res() {
    Some(Ok(content)) => {

...
impl EngineContext {
  pub fn new(site_config: SiteConfig, asset_root: PathBuf) -> Self {
    Self { site_config: Arc::new(site_config), asset_root }
  }
...
impl EngineRegistry {
  ...
  pub fn get<E: Engine + 'static>(&self, name: &str) -> Option<&E> {
    ...
    self.engines.get(idx).and_then(|e| e.as_any().downcast_ref::<E>())
  }

  /// 按类型获取引擎可变引用。`name` 不存在或类型不匹配 → `None`。
  pub fn get_mut<E: Engine + 'static>(&mut self, name: &str) -> Option<&mut E> {
    let idx = *self.name_to_index.get(name)?;
    self.engines.get_mut(idx).and_then(|e| e.as_any_mut().downcast_mut::<E>())
  }
...
mod tests {
  ...
  impl CountingEngine {
    ...
    fn failing(name: &'static str) -> Self {
      Self { fail_init: true, ..Self::new(name) }
    }
  }
  ...
  fn get_mut_allows_mutation() {
    ...
    let engine = reg.get_mut::<CountingEngine>("m").expect("engine should exist");
    // 直接调用 init 增加计数
    let ctx = EngineContext::for_tests();
